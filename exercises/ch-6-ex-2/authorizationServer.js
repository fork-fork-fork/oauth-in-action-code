...
app.post("/token", function(req, res){
	...
	// 토큰 엔드포인트의 핸들러에서 인가 코드 그랜트 타입의 요청을 처리
	if (req.body.grant_type == 'authorization_code') {
		...
		// 해당 핸들러에 클라이언트 자격 증명 그랜트 타입 처리 조건문 추가
	} else if (req.body.grant_type == 'client_credentials') {

		// 권한 범위만 확인(권한 범위를 확인하는 코드는 이전의 인가 코드 그랜트 타입에서 사용된 것을 그대로 사용)
		...
		// 확인 작업 이후, 액세스 토큰 발급
		// 발급하는 액세스 토큰은 앞의 경우와 마찬가지로 DB에 저장
		var access_token = randomstring.generate();
