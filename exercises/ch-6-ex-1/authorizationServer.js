...
app.post('/approve', function(req, res) {
	...
	if (req.body.approve) {
		if (query.response_type == 'code') {
			...
			res.redirect(urlParsed);
			return;
		} else if (query.response_type == 'token') { // response_type이 token인 경우를 처리하는 코드 블록 추가
			// 인가 코드 그랜트에서 토큰을 요청했을 때 처리하는 것과 동일한 작업 수행
			// - 요청된 권한 범위 등 확인
			// - 에러가 발생했을 때 에러 값은 query 파라미터가 아닌 hash 파라미터로 전달됨
            ...
            // 리다이렉트 uri의 hash 프레그먼트를 이용해 액세스 토큰을 클라이언트에게 전달
			var urlParsed = buildUrl(query.redirect_uri,
				{},
